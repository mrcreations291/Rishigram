...
export function CreateStoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (storyError) {
        setError(storyError.message || 'Error creating story');
        setIsUploading(false);
        return;
      }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg overflow-hidden dark:bg-gray-800 dark:border-gray-700">
        {!preview ? (
          <div 
            className="flex flex-col items-center justify-center h-80 cursor-pointer bg-gray-50 dark:bg-gray-900"
            onClick={() => fileInputRef.current?.click()}
          >
            <Image size={64} className="text-gray-400 mb-4" />
            <p className="text-gray-600 mb-2 dark:text-gray-400">Click to select an image for your story</p>
